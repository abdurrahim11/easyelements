...
class Options {
    ...
    function options_megamenu() {
        ...
        $data    = ele_get_option( Init::$megamenu_settings_key, array() );
    ...
    public function save_megamenu_options() {
        ...
        $ele_is_enabled = isset( $_POST['ele_is_enabled'] ) ? wp_unslash(intval($_POST['ele_is_enabled'])) : 0;

        $data                                = ele_get_option( Init::$megamenu_settings_key, array() );
        $data[ 'menu_location_' . $menu_id ] = array(
            'ele_is_enabled' => $ele_is_enabled,
        );

        ele_save_option( Init::$megamenu_settings_key, $data );
    }
}