...
class Elementor_Editor_Api  extends Handler_Api {
    ...
    public function get_content_editor() {
        ...
        $builder_post_id    = ele_get_page_by_title( $builder_post_title, 'easyelements_content' );
    ...
    public function set_wpml_data($builder_post_id) {
        ...
        if( !empty($referer_args['post']) ) {
            ...
            if( !is_wp_error($language_details) ) {
                $builder_post_id = apply_filters( 'wpml_object_id', $builder_post_id, 'easyelements_content', true, $language_details['language_code'] );
            }
        }
