...
class Library_Source extends Source_Local {
    ...
    public function import(array $args, $context = 'display') {
        ...
        if (!is_array($template_result) || !isset($template_result['json_url'])) {
            throw new \Exception(esc_html__('Template missing', 'easyelements'));
        }
        ...
        if (empty($data) || empty($data['content'])) {
            throw new \Exception(esc_html__('Template does not have any content', 'easyelements'));
        }
            ...
            foreach ($template_result['dependencies'] as $dependency) {
                ...
                if (!$this->is_plugin_installed( dirname( $plugin_slug ) )) {
                    ...
                    if (is_wp_error($install_result)) {
                        throw new \Exception(esc_html__('Error installing plugin', 'easyelements'));
                    }
                }
                ...
                if (!is_plugin_active( $plugin_slug )) {
                    ...
                    if (is_wp_error($activation_result)) {
                        throw new \Exception(esc_html__('Error activating plugin', 'easyelements'));
                    }
                    $this->dependencies++;
                }
            }
